...
const mixin = {
    data: () => ({
        mapOptions: {
            ...
            transparent: true,
            version: '1.1.1',
            attribution: "DeeMap Version 2.20.0 - Copyright © 2018 - 2020 - Powered By Thai Digital Map"
        },
        params: {
            id: 'deemapnigth'
        },
    ...
    methods: {
        ...
        async getMethod(servicename, queryValue = {}) {
            const returnData = await serviceProvider.getMethod(servicename, queryValue)
            if (returnData.code) {
                this.$vs.notify({
                    title: this.$t("error"),
        ...
        async postMethod(servicename, body = {}, query = {}) {
            const returnData = await serviceProvider.postMethod(servicename, body, query)
            if (returnData.code) {
                this.$vs.notify({
                    title: this.$t("error"),
                    text: this.$t(returnData.message),
        ...
        async postMethodWithFile(servicename, body = {}, query = {}) {
            const returnData = await serviceProvider.postMethodWithFile(servicename, body, query)
            if (returnData.code) {
                this.$swal({
