...
exports.upsert_equipment = async (req, res) => {
    try {
        ...
        let r1 = await pgcon.excutewithparams(sql, data, dbname, config.connectionString())
        if (r1.code) {
            res.send({ code: true, message: r1.message })
        } else {
            res.send({ code: false })
...
exports.add_new_group = async (req, res) => {
    try {
        ...
        if (r1.code) {
            ...
            data3.unshift(data2)
            let r3 = await pgcon.inserttransactionwithData(insertArr, data3, dbname, config.connectionString())
            if (r3.code) {
...
exports.edit_group_data = async (req, res) => {
    try {
        ...
        if (r1.code) {
            ...
            let dataArr = []
            //update gpms
            sqlArr.push(`UPDATE gpms SET g_name = $1 , last_config = NOW() WHERE g_id = $2`)
            dataArr.push([g_name, g_id])
            //delete gpeq
            sqlArr.push(`DELETE FROM gpeq WHERE g_id = $1`)
            dataArr.push([g_id])
            //insert gpeq
            let tempSql = eq_list.map(() => (`INSERT INTO gpeq(g_id,eq_id,time_config) VALUES ($1,$2,NOW())`))
            let tempData = eq_list.map(x => ([g_id, x]))
            sqlArr = sqlArr.concat(tempSql)
            dataArr = dataArr.concat(tempData)
            let r2 = await pgcon.inserttransactionwithData(sqlArr, dataArr, dbname, config.connectionString())
            if (r2.code) {
...
exports.delete_group_data = async (req, res) => {
    try {
        ...
        sqlArr.push(`DELETE FROM gpeq WHERE g_id = $1`)
        dataArr.push([g_id])
        let r1 = await pgcon.inserttransactionwithData(sqlArr, dataArr, dbname, config.connectionString())
        if (r1.code) {
            res.send({ code: true, message: r1.message })
        } else {
            res.send({ code: false })
...
exports.get_group_with_video = async (req, res) => {
    try {
        ...
        let sql = `SELECT gpms.g_id, gpms.g_name, vdo_arr, last_update FROM gpms LEFT JOIN gvdo ON gvdo.gid = gpms.g_id`
        let r1 = await pgcon.get(dbname, sql, config.connectionString())
        if (r1.code) {
            res.send({ code: true, message: r1.message })
        } else {
...
var slpUpload = multer({ storage: slpVideoDir, fileFilter: videoFilter }).single('vfile')

var slpUploadmulti = multer({ storage: slpVideoDir, fileFilter: videoFilter }).array('vfile')
    ...
    try {
        ...
        if (Array.isArray(files)) {
            for (let file of files) {
                ...
                let uploadErr = await file.mv(uploadPath + '/' + file.name)
                if (uploadErr) {
                    console.error(uploadErr);
                    return res.status(500).send(uploadErr);
                }
            }
            res.send({ code: false, data: files.map(x => x.name) })
        } else {
            files.mv(uploadPath + '/' + files.name, function (err) {
                if (err) {
                    console.error(err);
                    return res.status(500).send(err);
                }
                res.send({ code: false, data: [files.name] })
            })
        }
...
exports.matching_new_upload = async (req, res) => {
    try {
        ...
        if (r1.code) {
            ...
            let sqlArr = []
            let dataArr = []
            sqlArr.push(`INSERT INTO vdoms(v_id,file_name,g_id,start_date,end_date,status,upload_time) VALUES ($1,$2,$3,$4,$5,$6,NOW())`)
...
exports.update_video_status = async (req, res) => {
    try {
        ...
        let sqlArr = data.map(() => (`UPDATE vdoms SET seq = $1 , status = $2 WHERE v_id = $3 AND g_id = $4 `))
...
exports.delete_video = async (req, res) => {
    try {
        ...
        if (r1.code) {
            ...
            dataArr.push([v_id])
            if (seq) {
                let tempSql = nonDelVideo.map(() => (`UPDATE vdoms SET seq = $1 WHERE v_id = $2 AND g_id = $3`))
...
exports.create_new_user = async (req, res) => {
    try {
        ...
        let r1 = await pgcon.excutewithparams(sql1, data1, dbname, config.connectionString())
        if (r1.code) {
            res.send({ code: true, message: r1.message })
        } else {
            res.send({ code: false })
